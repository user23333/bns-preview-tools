	...
	public string ToString(TextArguments arguments, short AttributeCoefficient)
	{
		...
		for (int x = 1; x < arguments.Count; x++)
		{
			...
			arguments[x] = type switch
			{
				...
				ArgTypeSeq.Effect => $"<font name=\"00008130.Program.Fontset_ItemGrade_6\">{provider.GetTable<Effect>()[arg]?.Text}</font>",
				...
				ArgTypeSeq.Skill => $"<font name=\"00008130.Program.Fontset_ItemGrade_4\">{provider.GetTable<Skill3>()[arg]?.Text}</font>",
