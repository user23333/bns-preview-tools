...
public partial class HtmlDocument
{
	...
	public void LoadHtml(string html, TextArguments arguments = null)
	{
		ArgumentNullException.ThrowIfNull(html);
		Arguments = arguments;
	...
	internal HtmlNode CreateNode(HtmlNodeType type, int index, string name = null)
	{
		switch (type)
		{
			case HtmlNodeType.Element:
				return HtmlElementNode.CreateNode(name, this, index);
	...
	private void CloseCurrentNode()
	{
		...
		bool error = false;
		HtmlNode prev = Lastnodes.GetValueOrDefault(_currentnode.Name);
	...
	{
		HtmlNode resetter = Lastnodes.GetValueOrDefault(name);
		if (resetter == null)
			return null;
	...
	private void PushNodeStart(HtmlNodeType type, int index, int lineposition)
	{
		...
		if (type == HtmlNodeType.Element)
		{
			...
			while (_index < Text.Length)
			{
				...
				if (IsWhiteSpace(_c))
				{
					name = Text[startindex.._index];
					break;
				}

				if (_c == '>')
				{
					name = Text[startindex] == '/' ? Text[(startindex + 1).._index] :
						   Text[_index - 1] == '/' ? Text[startindex..(_index - 1)] :
		...
		_currentnode = CreateNode(type, index, name);
