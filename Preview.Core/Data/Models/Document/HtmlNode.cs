...
public partial class HtmlNode
{
	...
	static HtmlNode()
	{
		...
		//<br> see above
		ElementsFlags.Add("arg", HtmlElementFlag.Empty | HtmlElementFlag.Closed);
	...
	public T GetAttributeValue<T>(string name, T def = default)
	{
		return (T)GetAttributeValue(name, typeof(T), def);
	}
	...
	/// <param name="def">The default value to return if not found.</param>
	/// <returns>The value of the attribute if found, the default value if not found.</returns>
	public object GetAttributeValue(string name, Type type, object def)
	{
		ArgumentNullException.ThrowIfNull(name);
	...
	public HtmlAttribute SetAttributeValue(string name, object value)
	{
		ArgumentNullException.ThrowIfNull(name);
		...
		if (att == null)
		{
			return Attributes.Append(_ownerdocument.CreateAttribute(name, value.ToString()));
		}
	...
	internal void CloseNode(HtmlNode endnode, int level = 0)
	{
		...
		if (!Closed)
		{
			...
			HtmlNode self = _ownerdocument.Lastnodes.GetValueOrDefault(Name);
			if (self == this)
			{
