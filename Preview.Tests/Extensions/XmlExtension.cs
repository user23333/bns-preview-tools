...
public static class XmlExtension
{
	...
	public static void Write(this XElement parent, IUStruct stru, string typeName = null)
	{
		...
		var element = parent.AddElement(typeName ?? stru.GetType().Name);
		...
		foreach (var prop in stru.GetType().GetProperties().Where(p => p.CanWrite))
		{
			...
			else if (value.GetAttribute<StructFallback>() != null && value is not TintColor tint)
			{
				if (value is StringProperty sp && string.IsNullOrEmpty(sp.LabelText.Text)) continue;

				element.AddElement($"{element.Name}.{name}").Write((IUStruct)value, prop.PropertyType.Name);
			}
			else
			{
