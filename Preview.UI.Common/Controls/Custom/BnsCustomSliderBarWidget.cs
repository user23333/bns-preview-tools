...
public class BnsCustomSliderBarWidget : BnsCustomRangeBaseWidget
{
	...
	private class BnsCustomSliderBarWidgetGesture : InputGesture
	{
		...
		{
			if (targetElement is Slider slider &&
				inputEventArgs is KeyEventArgs keyEventArgs && Keyboard.Modifiers == ModifierKeys.None)
			{
	...
	private void UpdateMarkerPositionAndSize()
	{
		...
		if (SliderOrientation == Orientation.Orient_Vertical)
		{
			...
			LayoutData.SetOffsets(Marker, offset with { Top = (float)pos });
		}
		else
		{
			var pos = bReverseDirection ?
			...
			LayoutData.SetOffsets(Marker, offset with { Left = (float)pos });
		}
	}
	#endregion
}