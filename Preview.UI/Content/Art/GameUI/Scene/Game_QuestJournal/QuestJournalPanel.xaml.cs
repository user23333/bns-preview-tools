...
public partial class QuestJournalPanel
{
	...
	private void QuestJournal_QuestInfoHolder_DataChanged(object sender, DependencyPropertyChangedEventArgs e)
	{
		...
		if (QuestJournal_QuestInfo_Holder.SetVisiable(reward != null))
		{
			static void UpdateValue(BnsCustomLabelWidget label, int value, float rate = 1.0F)
			{
				((UIElement)label.Parent).SetVisiable(value > 0);
				label.String.LabelText = (value * rate).ToString();
			}
			...
			UpdateValue(QuestJournal_RewardInfo_Final_Duel_Point_Value, reward.BasicDuelPoint);
			UpdateValue(QuestJournal_RewardInfo_Final_PartyBattle_Point_Value, reward.BasicPartyBattlePoint);
			UpdateValue(QuestJournal_RewardInfo_Final_FieldPlay_Point_Value, reward.BasicFieldPlayPoint);
			UpdateValue(QuestJournal_RewardInfo_Final_Guild_Reputation_Value, reward.BasicGuildReputation);
			UpdateValue(QuestJournal_RewardInfo_Final_FixedReward_Reputation, reward.BasicFactionReputation);
			UpdateValue(QuestJournal_RewardInfo_Final_Production_Value, reward.BasicProductionExp);
			UpdateValue(QuestJournal_RewardInfo_Final_Masterty_Name, reward.BasicMasteryLevel);
		...
		#region BonusReward
		var settings = quest.Provider.GetTable<QuestBonusRewardSetting>().Where(record => record.Quest == quest);
		if (QuestJournal_RewardInfo_AdditionalReward.SetVisiable(settings.Any()))
		{
			// TODO: 
			settings.ForEach(x => Debug.WriteLine("QuestBonusRewardSetting: {0}", x.Attributes));

			var QuestBonusRewardSetting = settings.First();
			QuestJournal_AdditionalReward_Dungeon.Visibility = Visibility.Collapsed;
			QuestJournal_AdditionalReward_Dungeons.Visibility = Visibility.Collapsed;
			QuestJournal_AdditionalReward_Dungeons_level.Visibility = Visibility.Collapsed;
	...
	private static void UpdateItem(BnsCustomImageWidget widget, ModelElement slot, short count, sbyte SkillVarIdx = 0)
	{
		// check null
		if (!widget.SetVisiable(slot is not null)) return;
