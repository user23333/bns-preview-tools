...
internal class AuctionPanelViewModel : ObservableObject
{
	...
	public IEnumerable<MarketCategory2Group> MarketCategory
	{
		get
		{
			...
			{
				MarketCategory2Group.Add(new MarketCategory2Group.Favorite() { Visible = true, Name = "UI.Market.Category.Favorites".GetText() });

				foreach (var (category2, category3) in SequenceExtensions.MarketCategory2Group())
				{
					MarketCategory2Group.Add(new MarketCategory2Group.MarketCategory2()
					{	
						SortNo = 100,
						Visible = true,
						Name = category2.GetText(),
						Marketcategory2 = category2,
						MarketCategory3Group = category3?.Select(x => new Ref<MarketCategory3Group>(new MarketCategory3Group()
						{
							Visible = true,
							Name = x.GetText(),
							MarketCategory3 = [x]
