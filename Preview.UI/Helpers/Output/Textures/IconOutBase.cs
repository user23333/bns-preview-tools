...
public abstract class IconOutBase : IDisposable
{
	...
	{
		// init table
		db = new BnsDatabase(DefaultProvider.Load(UserSettings.Default.GameFolder, DatSelectDialog.Instance));
	...
	public void Output(string format, CancellationToken cancellationToken)
	{
		...
		cancellationToken.ThrowIfCancellationRequested();
		Output(provider, format, new ProgressTest(), cancellationToken);
	}

	protected abstract void Output(DefaultFileProvider provider, string format, IProgress<int> progress, CancellationToken cancellationToken);
...
public class ProgressTest : Progress<int>
{
	public int Total;
